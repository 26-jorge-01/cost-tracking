...
MODALITIES = {
    "Integrales": {
        ...
        "column_mapping": {
            "A": 1,   # REGIONAL <- Regional UDS (Col 1)
            "B": 2,   # CENTRO ZONAL <- Centro Zonal UDS (Col 2)
            "C": 3,   # MUNICIPIO <- Municipio UDS (Col 3)
            "D": 9,   # Componente para la UDS <- Componente para la UDS (Col 9)
            "E": 8,   # SERVICIO SIM <- SERVICIO 2026 (Col 8)
            "F": 8,   # NOMBRE DEL SERVICIO <- SERVICIO 2026 (Col 8)
            "G": 0,   # ZONA 2026. <- TIPO (Col 0)
    ...
    "Comunitarios": {
        ...
        "header_row": None,
        "column_mapping": {
            "A": 0,   # REGIONAL <- Regional UDS (Col 0)
            "B": "const:",  # REFERENCIA / No. CONTRATO SECOP (Clear)
            "C": "const:",  # No. RP  (Clear)
            "D": 1,   # CENTRO ZONAL <- Centro Zonal UDS (Col 1)
            "E": 2,   # MUNICIPIO <- Municipio UDS (Col 2)
            "F": 7,   # NOMBRE DEL SERVICIO SIM (INFORMATIVO) <- Nombre Servicio (Col 7)
            "G": 7,   # NOMBRE DEL SERVICIO <- Nombre Servicio (Col 7)
            "H": "const:",  # CONSECUTIVO CONTRATO  (Clear)
            "I": 10,  # CUPOS POR UNIDAD <- Cupos (Col 10)
            "J": 11,  # CANTIDAD DE UDS <- Madres / Unds (Col 11)
            "K": 11,  # CANTIDAD DE MADRES POR UDS 2024 <- Madres / Unds (Col 11)
            "L": 11,  # CANTIDAD DE MADRES POR UDS 2025-2026 <- Madres / Unds (Col 11)
            "M": 10,  # CUPOS POR UNIDAD <- Cupos (Col 10)
            "N": "const:",  # MOMENTOS EN FAMILIA (Clear)
            "O": "const:",  # UNIDADES TOTALES POR SERVICIO A ADICIONAR (Clear)
            "Q": "const:",  # F PARA CALCULO (Clear)
            "R": "const:",  # TIEMPO INICIAL DEL CONTRATO (Clear)
            "S": "const:",  # TIEMPO A ADICIONAR (Clear)
            "X": 12,  # BASE ORIGINAL <- Abrev (Col 12)
            "Y": "const:2026", # VIGENCIA
            "AH": "const:", # FORMA DE CONTRATACIÓN (Clear)
            "AK": "const:", # NIT (Clear)
            "AM": "const:", # TIPO DE ORGANIZACIÓN (Clear)
            "AT": "const:", # FECHA DE FIN DEL CONTRATO (Clear)
            "AY": "const:", # VALOR INICAL DEL SERVICIO 2024-2025-2026 (Clear)
            "BD": "const:", # VALOR INICIAL OTROS CONCEPTOS (Clear)
            "BF": "const:", # VALOR INICIAL CONTRAPARTIDA (Clear)
            "CC": "const:", # CANTIDAD TALENTO... (Clear)
            "CD": "const:", # CANTIDAD TALENTO... (Clear)
            "CE": "const:", # VALOR MOMENTOS... (Clear)
            "CG": "const:", # VALOR ADICIÓN... (Clear)
            "CJ": "const:0.05", # % APORTE EAS - VALOR CONTRAPARTIDA
        }
    }
}
